...
def find_file_with_extension(directory, extension):
    # 遍历指定目录及其子目录，查找特定类型的文件
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith(extension):
                return os.path.join(root, file)  # 返回找到的文件的完整路径
    return None  # 如果未找到符合条件的文件，返回 None

def get_gpt_sovits_tts_models(model_id):
    model_dir = model_id

    # 查找以 .ckpt 结尾的文件（GPT 模型文件）
    gpt_path = find_file_with_extension(model_dir, '.ckpt')

    # 查找以 .pth 结尾的文件（SoVITS 模型文件）
    sovits_path = find_file_with_extension(model_dir, '.pth')

    # 查找以 .wav 结尾的文件（音频参考文件）
    audio_refer_path = find_file_with_extension(model_dir, '.wav')

    # 查找以 .txt 结尾的文件（音频文本参考文件）
    audio_refer_text_path = find_file_with_extension(model_dir, '.txt')
...
def extract_file_name(file_path):
    """
    从文件路径中提取文件名（包括扩展名）并返回。

    参数:
    file_path (str): 完整的文件路径字符串。
...
class GPT_SoVITS_TTS_inference(object):
    def __init__(self,
                 prompt_language="en",
                 base_model_id=r"/root/GPT_SoVITS_Inference/GPT_SoVITS/Pretrained_Models",
                 audio_model_id=r"/root/GPT_SoVITS_Inference/GPT_SoVITS/Finetune_Weights"):
    ...
    def get_tts_wav(self, text, prompt_text, model_name="test", text_language="en",
                    how_to_cut="凑四句一切", 
                    ...
                    ref_free=True):  # 在不知道参考音频文本的情况下进行推理
        
        # ====== 函数内变量 ======
        if model_name in (None, "xxx"):
            raise ValueError("模型名称不允许为 None 或 'xxx'，终止推理。")

        wav_save_path = f"GPT_SoVITS/Voice_Output/{model_name}.wav"

        # ===
        # 根据声色指定相关模型与参考语音
        ref_wav_path = self.ref_wav_path

        # 强行指定其refer_free = True，如果模型在refer_free = False时出现吞字情况
        # 可更改列表中的名字，以适配训练好的不同模型
